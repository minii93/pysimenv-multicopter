...
from pymulticopter.base import MulticopterDyn
...
class BSControl(SimObject):
    ...
    """
    def __init__(self, alpha, m, J, **kwargs):
        super(BSControl, self).__init__(**kwargs)
    ...
    # implement
    def _forward(self, dyn: MulticopterDyn, sigma_d: np.ndarray, sigma_d_dot: np.ndarray = np.zeros(4)):
        """
...
class FLVelControl(SimObject):
    ...
    def __init__(self, m, J,
                 k_p_att: np.ndarray, k_d_att: np.ndarray, k_p_vel: np.ndarray, **kwargs):
        super(FLVelControl, self).__init__(**kwargs)
        self.m = m
        self.J = J  # (3, 3) array
    ...
    # implement
    def _forward(self, dyn: MulticopterDyn, v_d: np.ndarray = np.zeros(3)):
        """
        :param dyn: multi-copter dynamics
        ...
        u_t_1, u_t_2, u_t_3 = self.k_p_vel*(v_d - v)[:]

        f = self.m*np.sqrt(u_t_1**2 + u_t_2**2 + (u_t_3 - FlatEarthEnv.grav_accel)**2)
        phi_d = np.arcsin(self.m/f*u_t_2)
        theta_d = np.arctan(u_t_1/(u_t_3 -FlatEarthEnv.grav_accel))

        eta_d = np.array([phi_d, theta_d, 0.])
        u_star = self.k_p_att*(eta_d - eta) - self.k_d_att*omega
        tau = self.J.dot(u_star) + np.cross(omega, self.J.dot(omega))
...
class QuaternionAttControl(SimObject):
    ...
    """
    def __init__(self, J: np.ndarray, K: np.ndarray, **kwargs):
        super(QuaternionAttControl, self).__init__(**kwargs)
        self.J = J  # (3, 3) array
        self.K = K  # (3, 6) array

    # implement
    def _forward(self, dyn: MulticopterDyn, q_d: np.ndarray, omega_d: np.ndarray = np.zeros(3)) -> np.ndarray:
        q = dyn.quaternion
        omega = dyn.ang_vel
...
class QuaternionPosControl(SimObject):
    ...
    def __init__(self, m: float, K: np.ndarray, **kwargs):
        super(QuaternionPosControl, self).__init__(**kwargs)
        self.m = m
        self.K = K

    # implement
    def _forward(self, dyn: MulticopterDyn, p_d: np.ndarray, v_d: np.ndarray = np.zeros(3)):
        x = np.hstack((dyn.pos, dyn.vel))
...
class OAFControl(SimObject):
    ...
    2021.
    """
    def __init__(self, m, J, D_v, D_omega, G, K_p, K_d, **kwargs):
        super(OAFControl, self).__init__(**kwargs)
        self.m = m
        self.J = J
        self.d_v = np.diag(D_v)
        self.d_omega = np.diag(D_omega)
        self.G = G
        self.K_p = K_p  # P gains for [x, y, z, phi, theta]
        self.K_d = K_d  # D gains for [x, y, z, phi, theta]

    # implement
    def _forward(self, dyn: MulticopterDyn, fault_ind: int, p_d, p_d_dot = np.zeros(3), p_d_2dot = np.zeros(3)):
        p = dyn.pos
        v = dyn.vel
        eta = dyn.euler_ang
        omega = dyn.ang_vel
        ...
        w = np.hstack((w_r[0:fault_ind], np.array([0.]), w_r[fault_ind:]))
        return w
...
class OAFAttControl(SimObject):
    ...
    """
    def __init__(self, m, J, D_v, D_omega, G, K_p, K_d, **kwargs):
        super(OAFAttControl, self).__init__(**kwargs)
        self.m = m
        self.J = J
        self.d_v = np.diag(D_v)
        self.d_omega = np.diag(D_omega)
    ...
    # implement
    def _forward(self, dyn: MulticopterDyn, fault_ind: int, zeta_d, zeta_d_dot = np.zeros(3), zeta_d_2dot = np.zeros(3)):
        # zeta = [phi, theta, z]
        eta = dyn.euler_ang
